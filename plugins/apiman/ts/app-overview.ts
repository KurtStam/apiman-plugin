...
module Apiman {
  ...
  export var AppOverviewController = _module.controller("Apiman.AppOverviewController", ['$scope', '$location', 'OrgSvcs',  ($scope, $location, OrgSvcs) => {
    ...
    OrgSvcs.get({organizationId: params.org, entityType: 'applications', applicationId: params.app}, function(app) {
        $scope.app = app;
    } , function(error) {
        alert("ERROR=" + error);
    });    
    ...
    OrgSvcs.query({organizationId: params.org, entityType: 'applications', applicationId: params.app, versionsOrActivity: 'versions', version: $scope.version}, function(versions) {
